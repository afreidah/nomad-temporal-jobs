---
title: "GitHub Token Renewer Workflow"
linkTitle: "GitHub Token Renewer"
weight: 80
---

Keeps every managed repository's CI/release token secret continuously valid. The workflow reads the repo list from Consul, then for each repo mints a short-lived GitHub App installation token and writes it to the repo's Actions secret with bounded concurrency. Because the token is re-minted on every scheduled run, the secret never expires &mdash; this replaces hand-rotated Personal Access Tokens. A per-repo failure is recorded and the run continues; the workflow returns an error only after every repo has been attempted. **Hover over any step** for implementation details.
...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([RenewTokens<br/>Workflow]):::workflow --> DEFAULTS[Apply Config<br/>Defaults]:::workflow',
    '    DEFAULTS --> LIST[List Repos<br/>Consul KV]:::activity',
    '    LIST --> EMPTY{Repos<br/>Found?}:::decision',
    '    EMPTY -->|none| DONE',
    '    EMPTY -->|yes| ACQUIRE[Acquire Slot<br/>sem = Concurrency]:::workflow',
    '    ACQUIRE --> RENEW[Renew Repo Token<br/>mint + seal + set]:::activity',
    '    RENEW --> GATE{Renew<br/>OK?}:::decision',
    '    GATE -->|ok| RECORD[Record<br/>Result]:::workflow',
    '    GATE -->|failed| TRACK[Record Failure<br/>continue run]:::error',
    '    RECORD --> MORE{More<br/>Repos?}:::decision',
    '    TRACK --> MORE',
    '    MORE -->|yes, slot frees| ACQUIRE',
    '    MORE -->|no| JOIN[Join Fan-Out<br/>WaitGroup]:::workflow',
    '    JOIN --> ANYFAIL{Any<br/>Failed?}:::decision',
    '    ANYFAIL -->|yes| FAIL[Return Error<br/>with failures]:::error',
    '    ANYFAIL -->|no| DONE([Workflow<br/>Complete]):::workflow',
    '    FAIL --> DONE',
    '',
  ...
  mermaid.render('ghtokenrenewer-mermaid-svg', diagramSrc).then(function(result) {
  ...
  var nodeInfo = {
    START: {
      title: 'RenewTokens Workflow',
      badge: 'workflow', badgeText: 'workflow entry',
      body: '<p>Keeps each managed repo\'s CI/release token secret continuously valid by minting a fresh GitHub App token every run &mdash; replacing expiring Personal Access Tokens.</p><p>Receives <code>RenewConfig</code> with <code>Concurrency</code> from the schedule input. Runs on the github-token-renewer worker / <code>github-token-renewer-task-queue</code>.</p>'
    },
    DEFAULTS: {
      title: 'Apply Config Defaults',
      badge: 'workflow', badgeText: 'workflow logic',
      body: '<p>Fills unset fields so every activity sees a deterministic config across replay:</p><p><code>Concurrency</code>: <code>4</code> &mdash; a zero would size the semaphore at 0 and deadlock, so the default is forced positive.</p>'
    },
    LIST: {
      title: 'List Repos',
      badge: 'activity', badgeText: 'activity',
      body: '<p><code>ListRepos</code> reads the newline-separated <code>owner/repo</code> list from Consul KV (<code>github/token-renewer/repos</code>), skipping blank lines and <code>#</code> comments, through the shared instrumented Consul client. A missing key is non-retryable &mdash; the operator must seed it.</p>'
    },
    EMPTY: {
      title: 'Repos Found?',
      badge: 'decision', badgeText: 'empty check',
      body: '<p>If the list is empty, the fan-out is skipped and the workflow completes successfully with an empty result.</p>'
    },
    ACQUIRE: {
      title: 'Acquire Slot (semaphore)',
      badge: 'workflow', badgeText: 'bounded concurrency',
      body: '<p>The fan-out is gated by a buffered channel sized to <code>Concurrency</code>. Each repo\'s goroutine sends into the channel to acquire a slot before renewing and receives from it on completion, so at most <code>Concurrency</code> repos are processed at once and a large fleet never bursts the GitHub API.</p>'
    },
    RENEW: {
      title: 'Renew Repo Token (per repo)',
      badge: 'activity', badgeText: 'activity, per-repo',
      body: '<p><code>RenewRepoToken</code> mints a short-lived GitHub App installation token scoped to the repo with <code>contents</code> + <code>pull-requests: write</code> (deliberately not secrets), seals it with a NaCl anonymous box against the repo\'s Actions public key, and writes it to the <code>RELEASE_PAT</code> secret using a separate <code>secrets: write</code> token. 3 attempts with exponential backoff; an unparseable <code>owner/repo</code> is non-retryable.</p>'
    },
    GATE: {
      title: 'Renew OK?',
      badge: 'decision', badgeText: 'per-repo outcome',
      body: '<p>A single repo\'s failure does not abort the run. The outcome is recorded and the other repos are still renewed.</p>'
    },
    RECORD: {
      title: 'Record Result',
      badge: 'workflow', badgeText: 'workflow logic',
      body: '<p>Stores a <code>RepoRenewResult</code> (repo, token expiry) at this repo\'s index for the post-barrier summary.</p>'
    },
    TRACK: {
      title: 'Record Failure (continue)',
      badge: 'error', badgeText: 'non-fatal',
      body: '<p>Stores the repo\'s error in the per-index error slice, then keeps going. The run only fails <b>after</b> every repo has been attempted.</p>'
    },
    MORE: {
      title: 'More Repos?',
      badge: 'decision', badgeText: 'fan-out loop',
      body: '<p>Each completed renewal frees a semaphore slot for the next queued repo, until all have run.</p>'
    },
    JOIN: {
      title: 'Join Fan-Out (WaitGroup)',
      badge: 'workflow', badgeText: 'barrier',
      body: '<p>A <code>workflow.WaitGroup</code> blocks until every per-repo goroutine has finished, so results are partitioned into renewed / failed only after the barrier &mdash; no concurrent appends.</p>'
    },
    ANYFAIL: {
      title: 'Any Failed?',
      badge: 'decision', badgeText: 'aggregate',
      body: '<p>The per-index errors are joined with <code>errors.Join</code>. If any repo failed, the workflow reports overall failure; otherwise it succeeds.</p>'
    },
    FAIL: {
      title: 'Return Error (with failures)',
      badge: 'error', badgeText: 'aggregate error',
      body: '<p>Sets <code>Success=false</code> and returns the joined error naming each failed repo, alongside the renewed list and failed list for diagnostics.</p>'
    },
    DONE: {
      title: 'Workflow Complete',
      badge: 'workflow', badgeText: 'result',
      body: '<p>Returns <code>RenewResult</code> with the renewed repos (and token expiries), any failed repos, and an overall <code>Success</code> flag.</p>'
    }
  };
...
| <span style="color:#f85149">**Red**</span> | Error handling (non-fatal per repo) |
