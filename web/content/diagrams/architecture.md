...
(function() {
  var diagramSrc = [
    'flowchart LR',
    '    SCHED([Temporal Schedules<br/>Terraform-managed]):::entry --> TEMPORAL[Temporal<br/>Server]:::middleware',
    '',
    '    TEMPORAL --> BTQ[backup<br/>task-queue]:::middleware',
    '    TEMPORAL --> TTQ[trivy<br/>task-queue]:::middleware',
    '    TEMPORAL --> CTQ[cleanup<br/>task-queue]:::middleware',
    ...
    '    CTQ --> CWORKER[Cleanup<br/>Worker]:::handler',
    ...
    '',
    '    CWORKER --> NOMAD_API',
    '    CWORKER --> SSH[SSH / SFTP<br/>+ Docker tunnel]:::data',
    '    CWORKER --> PG',
    '',
    '    CEWORKER --> VAULT[Vault]:::data',
    '    CEWORKER --> ACME[ACME<br/>LetsEncrypt]:::data',
    '    CEWORKER --> CF[Cloudflare<br/>DNS]:::data',
    '',
    '    GWORKER --> VAULT',
    '    GWORKER --> CONSUL_API',
    '    GWORKER --> GITHUB[GitHub<br/>App API]:::data',
    '',
    '    RSWORKER --> VAULT',
    '    RSWORKER --> CONSUL_API',
    '    RSWORKER --> GITHUB',
  ...
  mermaid.initialize({
    ...
    flowchart: { nodeSpacing: 26, rankSpacing: 46, curve: 'linear', padding: 12, diagramPadding: 16, useMaxWidth: false, htmlLabels: true }
  });
  ...
  var nodeInfo = {
    ...
    RSTQ: {
      ...
      badge: 'middleware', badgeText: 'task queue',
      body: '<p>Temporal task queue for the runner-scaler workflows (<code>PollAndDispatch</code> parent + <code>HandleRunner</code> children) and their activities. Only the runner-scaler worker polls this queue.</p><p>Fired ~every 30s via a fine-grained calendar spec (<code>second=0,30</code>). Each tick reconciles by depth &mdash; it counts the runners already in flight per <code>(repo, labels)</code> bucket and starts children only for the shortfall &mdash; so a job still queued next tick simply tops the count back up, with no per-job dedup or external state store.</p>'
    },
    RSWORKER: {
      title: 'Runner Scaler Worker',
      badge: 'handler', badgeText: 'worker',
      body: '<p>Scales self-hosted CI runners on demand (zero idle). Each tick reads the per-repo config from Consul (<code>runners/config</code>), lists each repo\'s queued <code>self-hosted</code> Actions jobs on GitHub, buckets them by <code>(repo, labels)</code>, and starts children only for the shortfall over the runners already in flight &mdash; optionally clamped by a <code>maxConcurrent</code> cap.</p><p>Per repo it runs one of two modes: <code>app</code> repos are minted a registration token through the shared GitHub App (minted inside the activity, so it never enters workflow history); <code>vault</code> repos are polled with a PAT and dispatch a self-registering job, splitting the poll token from a higher-privilege registration token. Ordered profiles route a <code>runs-on</code> label to a specific runner pool. A backstop timer reaps a runner that never picked its job up.</p><p>Self-authenticates with its Nomad Workload Identity and pulls credentials through Vault &mdash; the token-renewer App key for <code>app</code> repos, per-repo PATs for <code>vault</code> repos. <a href="../runnerscaler-workflow/">Runner scaler workflow diagram &rarr;</a></p>'
    },
    GWORKER: {
    ...
    GITHUB: {
      ...
      badge: 'data', badgeText: 'external service',
      body: '<p>GitHub REST API, reached as a GitHub App (one app installed across all managed repos) via the native <code>go-github</code> client &mdash; no <code>gh</code> CLI.</p><p>The token renewer mints short-lived, repo-scoped installation tokens and writes repository Actions secrets (sealed against the repo\'s public key). The runner scaler reuses the same App to discover queued self-hosted jobs and mint registration tokens for <code>app</code>-mode repos; for <code>vault</code>-mode repos (e.g. ones the App can\'t be installed on) it uses per-repo PATs instead. An App is the only way to mint PR-capable tokens programmatically. Produces <code>peer.service: github</code> service-graph edges.</p>'
    },
    TEMPO: {
      title: 'Tempo (Tracing)',
