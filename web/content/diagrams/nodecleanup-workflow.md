...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([Cleanup<br/>Workflow]):::workflow --> DEFAULTS[Apply Config<br/>Defaults]:::workflow',
    '    DEFAULTS --> DISCOVER[Get Nomad<br/>Client Nodes]:::activity',
    '    DISCOVER --> EMPTY{Nodes<br/>Found?}:::decision',
    '    EMPTY -->|none| DONE',
    '    EMPTY -->|yes| LOOP[Process<br/>Next Node]:::workflow',
    '    LOOP --> RUNJOBS[Get Running Jobs<br/>Nomad API]:::activity',
    '    RUNJOBS --> CONNECT[Open SSH/SFTP<br/>to Node]:::activity',
    '    CONNECT --> LIST[List Dirs<br/>via SFTP]:::activity',
    '    LIST --> EXCLUDE{System<br/>Dir?}:::decision',
    '    EXCLUDE -->|yes| SKIP_DIR[Skip<br/>Directory]:::workflow',
    '    EXCLUDE -->|no| RUNNING{Matches<br/>Running Job?}:::decision',
    '    RUNNING -->|active| MORE',
    '    RUNNING -->|no| GRACE{Older Than<br/>Grace Period?}:::decision',
    '    GRACE -->|no| SKIP_DIR',
    '    GRACE -->|yes| DRYRUN{Dry Run<br/>Mode?}:::decision',
    '    DRYRUN -->|dry run| LOG_ONLY[Log Would<br/>Delete]:::workflow',
    '    DRYRUN -->|live| DELETE[Remove Dir<br/>via SFTP]:::activity',
    '    SKIP_DIR --> MORE{More<br/>Dirs?}:::decision',
    '    LOG_ONLY --> MORE',
    '    DELETE --> MORE',
    '    MORE -->|yes| EXCLUDE',
    '    MORE -->|no| DOCKER{Docker<br/>Prune?}:::decision',
    '    DOCKER -->|yes| PRUNE[Docker Prune<br/>Docker API]:::activity',
    '    DOCKER -->|no| CTRD{Containerd<br/>Prune?}:::decision',
    '    PRUNE --> CTRD',
    '    CTRD -->|yes| CTRDPRUNE[Containerd Prune<br/>store-aware]:::activity',
    '    CTRD -->|no| ROOTFS{rootfs<br/>Sweep?}:::decision',
    '    CTRDPRUNE --> ROOTFS',
    '    ROOTFS -->|yes| ROOTFSPRUNE[Rootfs Sweep<br/>SFTP + mounts]:::activity',
    '    ROOTFS -->|no| BUILDX',
    '    ROOTFSPRUNE --> BUILDX{buildx<br/>Volumes?}:::decision',
    '    BUILDX -->|yes| BUILDXPRUNE[Buildx Volume<br/>Prune]:::activity',
    '    BUILDX -->|no| TRACK',
    '    BUILDXPRUNE --> TRACK{Node<br/>Failed?}:::decision',
    '    TRACK -->|yes| FAIL[Track Failed<br/>Node]:::error',
    '    TRACK -->|no| NEXT',
    '    FAIL --> NEXT{More<br/>Nodes?}:::decision',
    '    NEXT -->|yes| LOOP',
    '    NEXT -->|no| REPORT[Report<br/>Totals]:::workflow',
    '    REPORT --> DONE([Workflow<br/>Complete]):::workflow',
    '',
