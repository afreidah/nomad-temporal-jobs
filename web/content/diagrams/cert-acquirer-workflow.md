...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([CertAcquirer<br/>Workflow]):::workflow --> ISSUE[Issue Wildcard Cert<br/>ACME DNS-01]:::activity',
    '    ISSUE --> IGATE{Issued<br/>OK?}:::decision',
    '    IGATE -->|rate-limited / fail| IFAIL[Return Error<br/>nothing published]:::error',
    '    IGATE -->|ok, staged in Vault| PUBLISH[Publish Cert<br/>to Vault]:::activity',
    '    PUBLISH --> PGATE{Published<br/>OK?}:::decision',
    '    PGATE -->|fail| PRETRY[Temporal retries<br/>up to 5x]:::workflow',
    '    PRETRY --> PUBLISH',
    '    PGATE -->|ok| DONE([Workflow<br/>Complete]):::workflow',
    '',
