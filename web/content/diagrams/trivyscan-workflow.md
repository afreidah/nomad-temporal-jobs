...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([Scan<br/>Workflow]):::workflow --> DISCOVER[Get Running<br/>Images]:::activity',
    '    DISCOVER --> EMPTY{Images<br/>Found?}:::decision',
    '    EMPTY -->|none| DONE',
    '    EMPTY -->|yes| BATCH[Create Batches<br/>of 10]:::workflow',
    '    BATCH --> LOOP[Process<br/>Next Batch]:::workflow',
    '    LOOP --> PARALLEL[Scan Images<br/>in Parallel]:::activity',
    '    PARALLEL --> COLLECT[Collect<br/>Results]:::workflow',
    '    COLLECT --> CLASSIFY{Error<br/>Type?}:::decision',
    '    CLASSIFY -->|success| SAVE[Save Scan<br/>Result]:::activity',
    '    CLASSIFY -->|transient| RETRY[Temporal<br/>Retries]:::error',
    '    CLASSIFY -->|permanent| SKIP[Log & Skip<br/>Image]:::error',
    '    RETRY --> SAVE',
    '    SKIP --> MORE',
    '    SAVE --> MORE{More<br/>Batches?}:::decision',
    '    MORE -->|yes| LOOP',
    '    MORE -->|no| SUMMARY[Log Aggregate<br/>CVE Counts]:::workflow',
    '    SUMMARY --> DONE([Workflow<br/>Complete]):::workflow',
    '',
