...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([PollAndDispatch<br/>Parent]):::workflow --> DEFAULTS[Apply Config<br/>Defaults]:::workflow',
    '    DEFAULTS --> LOADCFG[Load Config<br/>Consul runners/config]:::activity',
    '    LOADCFG --> ACQUIRE[Acquire Slot<br/>sem = Concurrency]:::workflow',
    '    ACQUIRE --> LISTJOBS[List Queued Jobs<br/>App or PAT, per repo]:::activity',
    '    LISTJOBS --> JOBGATE{List<br/>OK?}:::decision',
    '    JOBGATE -->|failed| SKIPREPO[Skip Repo<br/>this tick]:::error',
    '    JOBGATE -->|ok| MOREREPOS{More<br/>Repos?}:::decision',
    '    SKIPREPO --> MOREREPOS',
    '    MOREREPOS -->|yes, slot frees| ACQUIRE',
    '    MOREREPOS -->|no, all scanned| COUNT[Count Active Runners<br/>Nomad, per bucket]:::activity',
    '    COUNT --> BUCKET[Bucket Jobs<br/>by repo + labels]:::workflow',
    '    BUCKET --> RECONCILE{Shortfall?<br/>queued - active, capped}:::decision',
    '    RECONCILE -->|covered| DONE([Poll Complete<br/>summary]):::workflow',
    '    RECONCILE -->|each missing runner| STARTCHILD[Start Child<br/>runner-runid-seq]:::workflow',
    '    STARTCHILD --> DONE',
    '',
    '    STARTCHILD -.spawns.-> CSTART',
    '    CSTART([HandleRunner<br/>Child]):::workflow --> DISPATCH[Dispatch Runner<br/>mint token or Vault secret]:::activity',
    '    DISPATCH --> WAIT[Wait Runner Done<br/>backstop deadline]:::activity',
    '    WAIT --> REAP[Reap Runner<br/>stop Nomad job]:::activity',
    '    REAP --> CDONE([Child<br/>Complete]):::workflow',
    '',
