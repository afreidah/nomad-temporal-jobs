...
(function() {
  var diagramSrc = [
    'flowchart TD',
    '    START([PostgresMaintenance<br/>Workflow]):::workflow --> DEFAULTS[Apply Config<br/>Defaults]:::workflow',
    '    DEFAULTS --> LIST[List<br/>Databases]:::activity',
    '    LIST --> EMPTY{Databases<br/>Found?}:::decision',
    '    EMPTY -->|none| DONE',
    '    EMPTY -->|yes| ACQUIRE[Acquire Slot<br/>sem = Concurrency]:::workflow',
    '    ACQUIRE --> VACUUM[VACUUM ANALYZE<br/>per database]:::activity',
    '    VACUUM --> GATE{VACUUM<br/>OK?}:::decision',
    '    GATE -->|ok| RECORD[Record<br/>Outcome]:::workflow',
    '    GATE -->|failed| TRACK[Record Error<br/>continue run]:::error',
    '    RECORD --> MORE{More<br/>Databases?}:::decision',
    '    TRACK --> MORE',
