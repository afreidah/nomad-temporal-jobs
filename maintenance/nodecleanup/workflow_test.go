...
func TestCleanup_Success(t *testing.T) {
	...
	result1 := CleanupResult{NodeName: "node1", NodeAddr: "10.0.0.1", Scanned: 5, Orphaned: 1}
...
func TestCleanup_AppliesDefaults(t *testing.T) {
	...
	env.OnActivity(a.GetAllNomadClientNodes, mock.Anything).Return([]nodes.NodeInfo{}, nil)

	// Pass zero config — workflow should apply defaults
	env.ExecuteWorkflow(Cleanup, CleanupConfig{})
...
func TestCleanup_NodeDiscoveryFailure(t *testing.T) {
	...
	env.ExecuteWorkflow(Cleanup, CleanupConfig{DataDir: "/opt/nomad/data", GraceDays: 7})
...
func TestCleanup_NodeFailureReported(t *testing.T) {
	...
	config := CleanupConfig{DataDir: "/opt/nomad/data", GraceDays: 7, DryRun: true}
